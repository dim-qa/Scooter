...
class TestApi:
    ...
    @pytest.mark.parametrize('api, code, login, password', [
        (data.METHOD_CODE[0]['login'], "Недостаточно данных для входа", 'login', 'password'),
        (data.METHOD_CODE[0]['login'], "Недостаточно данных для входа", 'none', 'none')
    ])
    @patch('requests.post')
    def test_inner_without_param(self, mock_post, new_account, api, code, login, password):
        mock_post.return_value.json.return_value = {'message': "Недостаточно данных для входа"}
        account = new_account[0]
        account.pop(login, None)
    ...
    @patch('requests.post')
    def test_failed_inner_without_param(self, mock_post, new_account, api, code, account_value):
        mock_post.return_value.json.return_value = {'message': "Недостаточно данных для входа"}
        account = new_account[0]
